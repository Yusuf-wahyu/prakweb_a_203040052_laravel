...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="position-absolute px-3 py-2 " style="background-color : rgba(0, 0, 0, 8)">
                    <a href="/posts/{{ $post->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
                <img src="https://source.unsplash.com/500x400?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
                <div class="card-body">
                  <h5 class="card-title">{{ $post->category->name }}</h5>
                  <p>
                    <small class="text-muted">
                        By. <a href="/categories/{{ $post->category->slug }}" class="text-decoration-none">{{ $post->author->name }}</a>
